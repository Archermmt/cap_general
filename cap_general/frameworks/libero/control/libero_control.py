...
@dataclass
class LiberoControlConfig(BaseControlConfig):
    """Configuration for LiberoControl."""
...
class LiberoControl(BaseControl):
    """Control that runs LIBERO subtasks with a configured VLA policy."""

    control_type = "libero"
    config_cls = LiberoControlConfig
    ...
    def libero_vla_episode(self, task: str, max_steps: int = 300, policy_name: str = "policy::base") -> bool:
        ...
        for step_idx in range(max_steps):
            ...
            response = self._run_policy(policy_name=policy_name, inputs={"example": example, "step": step_idx})
