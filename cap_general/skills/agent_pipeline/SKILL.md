...
Selector tools use an `agents` list. `run_pipe` uses an `agent_options` mapping keyed by agent name or alias. Response keys use each agent's scene-visible mark, usually `alias(agent_name)` when an alias exists.

## Pipeline Request

Each agent's entry in `agent_options` requires:

- `policy_name`: Policy key to operate on. Read from `agent_doc` → `policy_doc`.
- `job_options`: Ordered list of job entries. Each entry has:
  - `job`: Job group name (e.g. `"train"`, `"eval"`, `"compile"`).
  - `options`: Job-specific runtime options (see `agent_doc` → `run_pipe` function doc for per-job options).
...
1. Identify the requested agent and parse the job request from the user.
2. Call `{cap_id}_agent_doc` to inspect `policy_doc` and the `run_pipe` function doc for available jobs and per-job options. Read `result["scene"]["async_task"]` and store it as `async_task`.
3. Select the policy and construct `job_options` from `agent_doc`. Ask the user only when multiple valid policies remain genuinely ambiguous.
...
- `running=false`: Inspect `result`. A successful pipeline result contains `ok=true` and a per-job `report`. Use a messaging tool to send the user the complete result, then stop polling that agent.
...
1. Trigger this skill whenever the user asks an agent to train, evaluate, compile, or run any pipeline stage.
2. Read `{cap_id}_agent_doc` before constructing the request; check `result["scene"]["async_task"]` and the `run_pipe` function doc for valid jobs and options.
