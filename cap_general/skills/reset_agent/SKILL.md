...
Reset selected agents by calling `{cap_id}_reset` with an `agent_options` mapping, then use each keyed result's `obs` object to display or analyze the primary image using an image-capable tool.
...
- Display or analyze the returned `main_image` using an image-capable tool
...
After reset, use an image-capable tool to display or analyze the returned `obs.main_image` (convert to absolute path if needed).

If `obs.main_image` is missing but `obs.images` contains camera images, use the first useful camera image from `obs.images`.
...
4. Use an image-capable tool to display or analyze the `obs.main_image`.
5. If `obs.main_image` is missing, use the first available path from `obs.images`.
...
3. **Display or analyze the returned primary image** using an image-capable tool.
4. **Use absolute paths when displaying images**. If reset returns a relative image path, convert it to an absolute local path first.
