...
- `train_agent`: Train one or more agents for a requested number of epochs, poll status every five seconds, and report every result with `message`.
...
3. **Use the skill-specific tool sequence.** For example, use `get_state` before calling `{cap_id}_get_obs`, `reset_agent` before calling `{cap_id}_reset`, `train_agent` before calling `{cap_id}_train`, and `execute_task` before calling `{cap_id}_execute` or `{cap_id}_retry`.
...
- User asks a robot or agent to train, learn, fine-tune, or continue training for a number of rounds or epochs: read `train_agent/SKILL.md`.
