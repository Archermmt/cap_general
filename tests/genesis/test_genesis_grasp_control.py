"""Test GraspControl locally or remotely through MCP.

Local task mode:
    python tests/genesis/test_genesis_grasp_control.py

Local train mode:
    python tests/genesis/test_genesis_grasp_control.py --train_ep 100

Remote task mode:
    capcmd server --config configs/genesis/genesis_grasp_control.yaml
    python tests/genesis/test_genesis_grasp_control.py --remote --config configs/genesis/genesis_grasp_control.yaml

Remote train mode:
    capcmd server --config configs/genesis/genesis_grasp_control.yaml
    python tests/genesis/test_genesis_grasp_control.py --remote --config configs/genesis/genesis_grasp_control.yaml --train_ep 100
"""
...
_DEFAULT_TASK_NUM = 4
_DEFAULT_CONFIG = "configs/genesis/genesis_grasp_control.yaml"
_DEFAULT_AGENT = "grasp"


def _make_code(max_steps: int) -> str:
    """Build GraspControl execution code with values baked in."""
    return f"""\
...
def _make_train_eval_request(train_ep: int, max_steps: int) -> dict:
    """Build a lightweight GraspControl train-then-eval request for smoke tests."""
    return {
        "job_options": [
...
def test_grasp_config_defaults_to_rl_stage() -> None:
    ...
    from cap_general.frameworks.genesis.control.genesis_grasp_control import GenesisGraspControlConfig

    assert GenesisGraspControlConfig().stage == "rl"
...
async def _run_local(
    ...
    """Run GraspControl task episodes or training in-process."""
...
async def _run_remote(
    ...
    """Run GraspControl task episodes or training through MCP."""
    ...
    async with streamablehttp_client(url) as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            await test_utils.call_tool(session, "reset", {"control_options": {_DEFAULT_AGENT: {}}})
            scene_doc = (await test_utils.call_tool(session, "control_doc", {"controls": [_DEFAULT_AGENT]}))["scene"]
            async_task = scene_doc.get("async_task", True)
            ...
            for task_idx in range(task_num):
                ...
                status = await test_utils.call_tool(
                    session,
                    "execute",
                    {"control_codes": {_DEFAULT_AGENT: _make_code(max_steps)}},
                )
                if async_task:
...
def run_grasp_test(
    ...
    """Run GraspControl task episodes or training in-process or through MCP."""
    if remote:
        if not config:
            raise ValueError("Remote GraspControl test requires --config")
...
def test_local_grasp_agent() -> None:
    """Smoke test: run a GraspControl episode in-process."""
    result = run_grasp_test(config=_DEFAULT_CONFIG)
    assert isinstance(result, dict)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Genesis GraspControl evaluation - local or MCP")
